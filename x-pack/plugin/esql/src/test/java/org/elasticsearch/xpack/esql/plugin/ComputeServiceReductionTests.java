...
import org.elasticsearch.xpack.esql.optimizer.rules.physical.PlanDeferredFetch;
...
public class ComputeServiceReductionTests extends ESTestCase {
    ...
    public void testPhysicalOptimizerPlansDistributedTopN() {
        PhysicalPlan optimizedPlan = distributedQueryPlan(
            "FROM employees | SORT hire_date | LIMIT 20 | KEEP hire_date, salary, emp_no",
            fetchConfiguration()
        );
        List<Map<String, List<String>>> concreteIndices = new java.util.ArrayList<>();
        PlannerUtils.forEachRelation(optimizedPlan, relation -> concreteIndices.add(relation.concreteIndices()));

        assertThat(concreteIndices, equalTo(List.of(Map.of(LOCAL_CLUSTER_GROUP_KEY, List.of("employees")))));
        assertThat(optimizedPlan.collect(FetchExec.class), hasSize(1));
        assertThat(optimizedPlan.collect(FetchBoundaryExec.class), hasSize(1));
    }
    ...
    public void testPlansDistributedTopNBeforeCoordinatorDataNodeSplit() {
        ...
        PhysicalPlan rewritten = applyDeferredFetchRule(distributedPlan, fetchConfiguration(), TransportVersion.current());

        assertTrue(rewritten.anyMatch(FetchBoundaryExec.class::isInstance));
    ...
    public void testNoRetainedContextsWhenFetchBoundaryIsNotProduced() {
        ...
        PhysicalPlan planned = applyDeferredFetchRule(physicalPlan, configuration, TransportVersion.current());
    ...
    public void testFetchBoundaryRequiresTransportVersion() {
        ...
        PhysicalPlan planned = applyDeferredFetchRule(
            physicalPlan,
            configuration,
    ...
    public void testFetchPlanningDoesNotDuplicateExistingDocAttribute() {
        ...
        EsRelation relation = new EsRelation(
            ...
            "employees",
            IndexMode.STANDARD,
            Map.of(LOCAL_CLUSTER_GROUP_KEY, List.of("employees")),
            Map.of(LOCAL_CLUSTER_GROUP_KEY, List.of("employees")),
        ...
        PhysicalPlan fetchPlan = applyDeferredFetchRule(distributedPlan, fetchConfiguration(), TransportVersion.current());
    ...
    public void testCoordinatorAndReducePlansUseFetchHandleSchema() {
        ...
        PhysicalPlan planned = applyDeferredFetchRule(distributedPlan, fetchConfiguration(), TransportVersion.current());
    ...
    private static void assertSpecializedAttributeIsNotFetchable(Attribute specialized, Configuration configuration) {
        ...
        PhysicalPlan planned = applyDeferredFetchRule(distributedPlan, configuration, TransportVersion.current());
        assertSame(distributedPlan, planned);
        assertFalse(planned.anyMatch(FetchBoundaryExec.class::isInstance));
    }
    ...
    private static void assertNoDeferredFetch(String query) {
        ...
        PhysicalPlan planned = applyDeferredFetchRule(physicalPlan, fetchConfiguration(), TransportVersion.current());
        assertThat(planned.collect(FetchExec.class), hasSize(0));
        assertFalse(planned.anyMatch(FetchBoundaryExec.class::isInstance));
    }

    private static PhysicalPlan applyDeferredFetchRule(
        PhysicalPlan distributedPlan,
        Configuration configuration,
        TransportVersion minimumTransportVersion
    ) {
        return new PlanDeferredFetch().apply(distributedPlan, new PhysicalOptimizerContext(configuration, minimumTransportVersion));
    }
    ...
    private static PhysicalPlan distributedQueryPlan(String query, Configuration configuration) {
        ...
        Map<String, IndexProperties> indexProperties = Map.of("employees", new IndexProperties(IndexMode.STANDARD, 0));
        Map<String, List<String>> resolvedIndices = Map.of(LOCAL_CLUSTER_GROUP_KEY, List.of("employees"));
        Analyzer analyzer = EsqlTestUtils.analyzer()
            .minimumTransportVersion(TransportVersion.current())
            .addIndex(new EsIndex("employees", mapping, indexProperties, resolvedIndices, resolvedIndices))
            .buildAnalyzer();
