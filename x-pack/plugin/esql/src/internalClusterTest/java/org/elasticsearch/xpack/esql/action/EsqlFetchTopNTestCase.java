...
public abstract class EsqlFetchTopNTestCase extends AbstractEsqlIntegTestCase {
    ...
    public void testFetchTopNWithoutNodeLevelReduction() {
        try (
            EsqlQueryResponse response = runQuery(
                "FROM " + indexName + " | SORT unique_sort + 1 DESC | LIMIT 5 | KEEP unique_sort, payload, category",
                true,
    ...
    public void testMultipleTopNsDoNotUseFetch() {
        try (
            EsqlQueryResponse response = runQuery(
                "FROM "
                    + indexName
    ...
    private EsqlQueryResponse runQuery(String query, boolean fetchTopN) {
        return runQuery(query, fetchTopN, true);
    }

    private EsqlQueryResponse runQuery(String query, boolean fetchTopN, boolean nodeLevelReduction) {
        // Keep the data path deterministic and make the node-reduction choice explicit.
