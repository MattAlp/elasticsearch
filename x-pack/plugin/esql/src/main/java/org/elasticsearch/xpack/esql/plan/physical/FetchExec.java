...
public class FetchExec extends BinaryExec implements EstimatesRowSize {
    ...
    public FetchExec(
        ...
        this.fetchPlan = validateFetchPlan(fetchPlan);
    ...
    private void validateExtractionSpecs() {
        if (attributesToFetch.size() != extractionSpecs.size()) {
            throw new IllegalArgumentException(
                "fetch attributes [" + attributesToFetch.size() + "] must match extraction specifications [" + extractionSpecs.size() + "]"
            );
        }
        for (int i = 0; i < attributesToFetch.size(); i++) {
            if (attributesToFetch.get(i).dataType() != extractionSpecs.get(i).dataType()) {
                throw new IllegalArgumentException(
                    "fetch attribute ["
                        + attributesToFetch.get(i)
                        + "] has type ["
                        + attributesToFetch.get(i).dataType().typeName()
                        + "] but extraction specification has type ["
                        + extractionSpecs.get(i).dataType().typeName()
                        + "]"
                );
            }
        }
    ...
    private FetchExec(StreamInput in) throws IOException {
        ...
        this.fetchPlan = validateFetchPlan(right());
    ...
    /**
     * Validates the only logical-plan family supported by deferred fetch execution.
     */
    public static FragmentExec validateFetchPlan(PhysicalPlan plan) {
