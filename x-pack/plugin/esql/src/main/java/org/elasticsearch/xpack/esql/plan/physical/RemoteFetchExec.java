...
public class RemoteFetchExec extends UnaryExec implements EstimatesRowSize {
    ...
    public RemoteFetchExec(Source source, PhysicalPlan child, Attribute handleAttribute, List<Attribute> attributesToFetch) {
        super(source, child);
    ...
    protected NodeInfo<RemoteFetchExec> info() {
        return NodeInfo.create(this, RemoteFetchExec::new, child(), handleAttribute, attributesToFetch);
    }

    @Override
    public RemoteFetchExec replaceChild(PhysicalPlan newChild) {
        return new RemoteFetchExec(source(), newChild, handleAttribute, attributesToFetch);
    }
    ...
    public List<Attribute> output() {
        if (lazyOutput == null) {
            ...
            lazyOutput = new ArrayList<>(childOutput.size() + attributesToFetch.size());
            lazyOutput.addAll(childOutput);
            lazyOutput.addAll(attributesToFetch);
        }
        return lazyOutput;
    }
    ...
    @Override
    public int hashCode() {
        return Objects.hash(child(), handleAttribute, attributesToFetch);
    }
